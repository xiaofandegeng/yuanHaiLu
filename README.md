...
- v4 存档：玩家身份/等级/位置/基础属性/当前 HP、MP、背包、装备、金钱、武学、活跃任务进度、已完成任务和正式外观。
- 新游戏、读档、普通运行使用显式场景进入模式，读档回调单次执行。
- 主菜单运行时补齐全局管理器并绑定按钮。
- 主菜单可选择 2 种性别 × 6 种职业；确认后才写入本次新游戏，取消不会污染当前外观。
- 97 套正式角色资源：12 主角、15 剧情角色、36 NPC、24 敌人、10 BOSS；全部有独立 PNG、动画 Controller 与 Prefab。
- 10 个户外区域、13 个室内场景、23 个正式环境场景；统一 7 层 Tilemap、地标、碰撞、前景遮挡、昼夜/天气、锚点和持久化 Tile 资源。
- 23 个正式场景已接通玩家/相机引导与传送图，所有场景都有可解析的后继场景和落点。
- 97 个 Controller 已接通四方向 idle/walk/dash/三段攻击；玩家和敌人都在动画命中帧结算伤害。
...
- v4 存档：在 v3 任务/背包/武学基础上增加正式主角外观稳定 ID，v1–v3 旧档迁移到男剑客。
...
当前运行时/编辑器代码为 71 个 C# 文件；另有 20 个测试/测试工具文件。

## 自动验证

项目使用 Unity Test Framework `1.6.0`，当前全量基线为 **92 个 EditMode + 10 个 PlayMode**；Python 美术流水线另有 **35 个测试**。覆盖：
...
- 97 个角色精确目录、Prefab/Controller、代表角色 idle→walk/attack 和真实命中帧；
- 23 个正式环境场景、7 层 Tilemap、碰撞/前景/昼夜/天气、旅行图和真实跨场景落点；
- 主菜单 12 套外观选择、菜单→Demo 外观保持、正式烟柳镇场景绑定；
- 主菜单→选角→移动→攻击→对话→暂停→存读档→传送的完整 E2E；
- 主菜单与 10 个户外区域 480×270 视觉像素基线；
...
| Tools/渊海录/美术/重建全部视觉基线 | 有意接受画面变化时重建 11 张基线 |
| Tools/渊海录/切分角色精灵表(32×32) | 角色切片 |
| Tools/渊海录/切分瓦片集(16×16) | 瓦片切片 |

## 主要待办

- 在当前模块化正式像素资源基础上继续人工精修细节、受击/倒地动作和区域差异。
...
正式美术重建与验收流程见 `docs/HANDOFF-art-production.md`。
